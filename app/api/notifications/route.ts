...
export const PUT = async (request: Request) => {
    try {
        ...
        await connect();
        const result = await Notification.updateMany({ user: new Types.ObjectId(id) }, { $set: { status: 'read' } });

        if (!result) {
            return new NextResponse(JSON.stringify({message: 'Failed to update notifications'}), {status: 400});
        }

        const notifications = await Notification.find({ user: id }).select('-password').sort({ createdAt: -1 });
